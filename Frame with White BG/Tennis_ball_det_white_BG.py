...
prev_position = None
prev_velocity = None
hit_velocity_threshold = 95  # Minimum speed change indicating a hit
...
# Function to detect the white background zone
def detect_white_zone(frame):
    # Convert frame to HSV color space
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    
    # Define range for white color (adjust these values as needed)
    lower_white = np.array([0, 0, 100])  # Lower bound for white in HSV
    upper_white = np.array([180, 70, 255])  # Upper bound for white in HSV
    
    # Create a mask for the white zone
    mask = cv2.inRange(hsv, lower_white, upper_white)
    
    # Find contours of the white zone
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    
    # Assume no white zone detected initially
    white_zone_contour = None
    
    if contours:
        # Get the largest contour by area, assuming it is the white background zone
        white_zone_contour = max(contours, key=cv2.contourArea)
        # Draw the detected white zone on the frame (for visualization)
        cv2.drawContours(frame, [white_zone_contour], -1, (255, 0, 0), 2)
...
# Function to check if the impact point is inside the detected white zone
def is_inside_white_zone(impact_point, white_zone_contour):
    if white_zone_contour is None:
        return False
    
    # Use pointPolygonTest to check if the point is inside the contour (-1=outside, 0=on, 1=inside)
    return cv2.pointPolygonTest(white_zone_contour, impact_point, False) > 0
...
while cap.isOpened():
    ...
    # Detect the white background zone
    white_zone_contour = detect_white_zone(frame)
    ...
    if ball_detected and impact_point:
        ...
        if is_hit(impact_point, prev_position, prev_velocity):
            # Check if the hit is inside the white zone
            if is_inside_white_zone(impact_point, white_zone_contour):
                ...
                print(f"Impact detected inside the white zone at: {virtual_point}")
            else:
                print("Impact outside the white zone - not considered.")
    ...
    cv2.resizeWindow("Live Feed", 640, 360)
